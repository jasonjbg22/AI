...
IGNORE_FILES = {
    "ai_watcher.py", "_ai_context.txt", "_ai_version_log.txt", 
...
GIT_IGNORE_PATTERNS = load_gitignore()

def is_ignored(path, is_dir=False):
    """Checks if a path matches built-in ignores or .gitignore patterns."""
    ...
    if is_dir:
        ...
        if any(name.endswith(ext) for ext in IGNORE_EXTS): return True
...
def generate_tree(startpath):
    ...
    for root, dirs, files in os.walk(startpath):
        ...
        subindent = ' ' * 4 * (level + 1)
        for f in files:
            if is_ignored(os.path.join(root, f)):
                continue
...
def rebuild_context_file():
        ...
        for f in files:
            ...
            try:
                ...
                if file_size > MAX_FILE_SIZE:
                    content_blocks.append(f"=== FILE: {rel_path} ===\n[SKIPPED: File too large ({file_size/1024:.1f} KB)]\n=== END FILE ===")
                    skipped_files.append(f"{rel_path} ({file_size/1024:.1f} KB)")
                    continue
            except OSError:
                continue
            ...
            # --- CONTENT READ ---
            try:
                with open(full_path, "r", encoding="utf-8", errors="strict") as file_obj:
    ...
    print(f"Context updated. (Size: {os.path.getsize(CONTEXT_FILE)/1024/1024:.2f} MB)")
    if skipped_files:
        print(f"Skipped {len(skipped_files)} large files (e.g., {skipped_files[0]})")


# =============================
# DEPLOY ENGINE (CLEAN WRITE)
# =============================
# (Kept identical to your original logic for stability)
